...
export function CommandPalette({ open, setOpen, handleStartTraining, handleAbortTraining, setActiveWorkspace }: CommandPaletteProps) {
  if (!open) return null;
        ...
        <Command label="Command Palette" onKeyDown={(e) => e.key === "Escape" && setOpen(false)}>
          ...
          <Command.List className="bg-[hsl(var(--panel))] border-x border-b border-[hsl(var(--border-muted))] rounded-b-lg overflow-y-auto max-h-[300px] custom-scrollbar p-2">
            ...
            <Command.Group heading="Execution" className="text-[10px] text-[#52525b] uppercase font-bold px-2 py-1">
              <CommandItem icon={Play} label="Execute Active Core" onSelect={() => { setOpen(false); handleStartTraining(); }} />
              <CommandItem icon={StopCircle} label="Terminate Running Pass" onSelect={() => { setOpen(false); handleAbortTraining(); }} />
            </Command.Group>
            <Command.Group heading="Workspace" className="text-[10px] text-[#52525b] uppercase font-bold px-2 py-1 mt-2">
              <CommandItem icon={Layers} label="Switch to Training" onSelect={() => { setOpen(false); setActiveWorkspace("Train"); }} />
              <CommandItem icon={Database} label="Open Library" onSelect={() => { setOpen(false); setActiveWorkspace("Library"); }} />
            </Command.Group>
          </Command.List>
        </Command>
      </div>
