...
export function TrainWorkspace() {
  const {
    metricsHistory, isRunning, progress, currentTrial, totalTrials, result, logs, clearLogs, loadedModelPath,
    split, setSplit, selectedPredictor,
    setMetricsHistory, setResult, setProgress, setTrialInfo, setIsRunning
  } = useTrainingStore();
  ...
  }, [loadedModelPath, selectedPredictor, clearLogs, setMetricsHistory, setResult, setProgress, setTrialInfo, setIsRunning]);
  ...
  const datasetName = selectedPredictor ? selectedPredictor.split('/').pop() : "None Selected";

  return (
    <div className="flex flex-col h-full bg-zinc-950 outline-none" tabIndex={-1}>
        ...
        <div className="h-12 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-900/50 shrink-0">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-zinc-500 uppercase">Split</span>
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-[10px] font-mono text-blue-500 w-8">{split}%</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <Panel defaultSize={70} minSize={30}>
                <div className="h-full overflow-y-auto custom-scrollbar p-6 space-y-6">
                  <div className="grid grid-cols-3 gap-4">
                    <SummaryCard icon={Table} label="Active Dataset" value={datasetName || "—"} subValue="Predictor Source" />
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-[11px] uppercase font-bold tracking-widest text-zinc-500">Live Metrics History</h3>
                      <div className="flex items-center gap-4 text-[10px] font-mono">
                        <span className="text-white flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-500"></div> Loss</span>
                        <span className="text-white flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500"></div> R²</span>
                      </div>
                    </div>
                    <div className="border border-zinc-800 rounded-lg p-4 bg-black min-h-[240px] flex items-center justify-center">
                      {isMounted ? (
                        ...
                        <div className="text-zinc-800 font-mono text-[10px]">Initializing Charts...</div>
                      )}
                    </div>
                  </div>

                  <div className="space-y-4">
                    <h3 className="text-[11px] uppercase font-bold tracking-widest text-zinc-500">Optimization Results</h3>
                    <div className="border border-zinc-800 rounded-lg overflow-hidden bg-zinc-900/30">
                      <table className="w-full text-left text-[11px]" suppressHydrationWarning>
                        <thead className="bg-zinc-900 text-zinc-500 font-bold">
                          <tr>
                            <th className="px-4 py-2 border-b border-zinc-800">TRIAL</th>
                            <th className="px-4 py-2 border-b border-zinc-800">R²</th>
                            <th className="px-4 py-2 border-b border-zinc-800">VAL_LOSS</th>
                            <th className="px-4 py-2 border-b border-zinc-800">MAE</th>
                          </tr>
                        </thead>
                        <tbody className="text-white font-mono">
                          {metricsHistory?.slice(-10).reverse().map((m: any, i: number) => (
                            <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                              <td className="px-4 py-2 text-zinc-500">#{m.trial}</td>
                              <td className="px-4 py-2 text-white">{m.r2?.toFixed(4) || "0.0000"}</td>
                ...
                <div className="h-full bg-zinc-900/50 flex flex-col overflow-hidden">
                  <div className="h-8 border-b border-zinc-800 flex items-center justify-between px-4 bg-black shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wider text-white">Process Stream</span>
                    </div>
                    <button 
                      onClick={clearLogs} 
                  ...
                  <div className="flex-1 overflow-y-auto p-3 font-mono text-[11px] leading-snug custom-scrollbar bg-black">
                    {logs?.map((log: any, i: number) => (
                      <div key={i} className="flex gap-3 mb-0.5 group">
                        <span className="text-zinc-800 shrink-0 tabular-nums">[{log.time}]{log.epoch !== null && log.epoch !== undefined ? ` (Epoch ${log.epoch})` : ''}</span>
                        <span className={log.type === 'success' ? 'text-green-500' : log.type === 'warn' ? 'text-yellow-500' : log.type === 'info' ? 'text-blue-500' : log.type === 'optuna' ? 'text-purple-500' : 'text-zinc-400'}>
                          {log.msg}
                        </span>
                      </div>
                    ))}
