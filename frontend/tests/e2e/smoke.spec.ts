...
test.describe("UI Stability Smoke Test", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on("console", (msg) => {
      if (msg.type() === "error") {
        ...
        if (
          !text.includes("[HMR]") &&
          !text.includes("Training start failed") &&
          !text.includes("Training already in progress")
        ) {
  ...
  async function testRegionButtons(
    page: Page,
...
--- Testing Region: ${regionName} ---`);
    ...
    const buttons = await region
      .locator('button:visible, [role="button"]:visible')
      .all();
    const identities: Array<{ type: "text" | "label" | "index"; value: string | number }> = [];
    ...
    for (const id of identities) {
      ...
      if (id.type === "text") {
        locator = scopedRegion
          .locator(`button, [role="button"]`)
          .filter({ hasText: id.value.toString() })
          .first();
      } else if (id.type === "label") {
        locator = scopedRegion.locator(`[aria-label="${String(id.value)}"]`).first();
      } else {
        locator = scopedRegion
          .locator('button:visible, [role="button"]:visible')
          .nth(Number(id.value));
      ...
      if (await locator.isVisible()) {
        try {
          ...
          if (!url.includes("localhost:3000")) {
            consoleErrors.push(
              `Navigation Error: Button "${btnName}" redirected to ${url}`,
            );
          }
          ...
          console.log(`⚠️ Soft Failure: Could not click "${btnName}": ${msg}`);
        }
      } else {
  ...
  test("Workflow: CNN Training and Log Capture", async ({ page }) => {
    ...
    const runBtn = page
      .locator("header")
      .locator("button", { hasText: "EXECUTE PASS" })
      .or(page.locator("header").locator("button", { hasText: "Run" }))
      .first();
    await runBtn.click();
